...
export async function constructUser(message) {
  ...
  const username = user?.nickname || message.author?.username;
  const nickname = user?.nickname || message.member?.displayName || message.author?.username;
  const server = guild?.name || message.guild?.name;

  return (usernames) => ({
    username: `${usernames ? username : nickname} from ${server}`,
    mdName: `**${usernames ? username : nickname}** from **${server}**`,
...
export async function constructMessage(message, replyDoc) {
  ...
  return (channel) => {
    ...
    const reply = message.type === DJS.MessageType.Reply && {
      author: replyMsg ?  replyUser(channel.usernames).mdName : '**[Original Not Found]**',
